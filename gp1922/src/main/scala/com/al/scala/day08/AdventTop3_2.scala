...
object AdventTop3_2 {
  def main(args: Array[String]): Unit = {
    ...
    //获取数据并切分
    val logsArr = sc.textFile("D://data/Advert.log").map(_.split("\t"))

    //将数据正和城元祖，便于整合，key= 省份+小时+广告id
    ...
    //用省份和小时进行分组
    val grouped: RDD[((String, String), Iterable[((String, String), String, Int)])] = proAndHourTup.groupBy(_._1)

    //开始组内排序
    val res: RDD[((String, String), List[((String, String), String, Int)])] = grouped.mapValues(x => x.toList.sortWith(_._3>_._3).take(3))
