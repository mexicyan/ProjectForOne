...
object LacLocation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //获取用户访问数据
    val logs = sc.textFile("d://data/log")
    ...
    //获取基站信息
    val lacInfo = sc.textFile("D://data/lac_info.txt")
        .map(line => {
          val fields = line.split(",")
          val lac = fields(0)
          val lang = fields(1)
    ...
    //按照手机号进行分组
    val grouped: RDD[(String, Iterable[(Long, (String, String))])] = phoneAndTimeAndLangLat.groupByKey()


    //组内排序
    val res: RDD[(String, List[(Long, (String, String))])] = grouped.mapValues(_.toList.sortWith(_._1 > _._1).take(2))
